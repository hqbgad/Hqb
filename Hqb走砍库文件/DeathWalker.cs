...
namespace DetuksSharp
{
    ...
    public class DeathWalker
    {
        ...
        {
            /*Config = new Menu("Hqb走砍", "dWalk", true);
        ...
        {
            menuIn.AddItem(new MenuItem("Combo_Key", "连招按键").SetValue(new KeyBind(32, KeyBindType.Press)));
            menuIn.AddItem(new MenuItem("Harass_Key", "骚扰按键").SetValue(new KeyBind('C', KeyBindType.Press)));
            menuIn.AddItem(new MenuItem("LaneClear_Key", "清线按键").SetValue(new KeyBind('V', KeyBindType.Press)));
            menuIn.AddItem(new MenuItem("LastHit_Key", "补刀按键").SetValue(new KeyBind('X', KeyBindType.Press)));
            menuIn.AddItem(new MenuItem("WindUp", "AA后摇时间").SetValue(new Slider(60, 0, 250)));
            menuIn.AddItem(new MenuItem("farmDelay", "打钱延迟").SetValue(new Slider(60, 0, 250)));
            menuIn.AddItem(new MenuItem("runCS", "启动CS距离").SetValue(new Slider(60, 0, 500)));

            menuIn.AddItem(new MenuItem("Sprite", "死亡走砍"));
            menuIn.AddItem(new MenuItem("Hanhua", "汉化人员:花边下丶情未央"));
            menuIn.AddItem(new MenuItem("wearethebest", "Best 最强系列 为你分享"));
            menuIn.AddItem(new MenuItem("duiwaiqqqun", "对外QQ群:299606556"));
