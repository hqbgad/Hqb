...
namespace RoyalAsheHelper
{
    class Program
    {
        ...
        {
            // Initialize the menu
            menu = new Menu("Hqb寒冰", champName, true);
            ...
            combo.AddItem(new MenuItem("UseQc", "使用Q").SetValue(true));
            combo.AddItem(new MenuItem("QmanaC", "最低使用Q蓝量")).SetValue(new Slider(0, 0, 100));
            combo.AddItem(new MenuItem("UseW", "使用W").SetValue(true));
            combo.AddItem(new MenuItem("WmanaC", "最低使用W蓝量")).SetValue(new Slider(15, 0, 100));
            combo.AddItem(new MenuItem("UseWE", "仅在离开AA范围使用W").SetValue(false));
            combo.AddItem(new MenuItem("UseR", "使用R").SetValue(true));
            combo.AddItem(new MenuItem("RSlider", "对敌人使用大招").SetValue(new Slider(3, 1, 5)));
            combo.AddItem(new MenuItem("forceR", "智能使用R").SetValue(new KeyBind('T', KeyBindType.Press)));

            // Harass
            Menu harass = new Menu("骚扰", "harass");
            menu.AddSubMenu(harass);
            harass.AddItem(new MenuItem("UseQh", "使用Q").SetValue(true));
            harass.AddItem(new MenuItem("UseWc", "使用W").SetValue(true));
            harass.AddItem(new MenuItem("WmanaH", "最低使用W蓝量")).SetValue(new Slider(50, 0, 100));

            // Harass
            Menu jc = new Menu("清野", "jc");
            menu.AddSubMenu(jc);
            jc.AddItem(new MenuItem("UseQj", "使用Q").SetValue(true));
            jc.AddItem(new MenuItem("UseWj", "使用W").SetValue(true));
            jc.AddItem(new MenuItem("manaJ", "最低蓝量%")).SetValue(new Slider(10, 0, 100));

            Menu misc = new Menu("杂项", "misc");
            menu.AddSubMenu(misc);
            misc.AddItem(new MenuItem("interrupt", "打断技能").SetValue(true));
            misc.AddItem(new MenuItem("antigapcloser", "反突脸").SetValue(true));

            var drawings = new Menu("标示", "Drawings");
            menu.AddSubMenu(drawings);
            drawings.AddItem(new MenuItem("Wrange", "W范围").SetValue(new Circle(true, Color.Cyan)));
            drawings.AddItem(new MenuItem("Ecircle", "R范围(连招)").SetValue(new Circle(true, Color.ForestGreen)));
